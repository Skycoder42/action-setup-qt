...
class Installer {
    constructor(version, platform) {
        ...
        switch (os.platform()) {
            case "linux":
                ...
                this._downloader = new downloader_1.default("linux", "x64", this._version, this._platform.platform);
                break;
            case "win32":
                ...
                this._downloader = new downloader_1.default("windows", "x86", this._version, this._platform.platform);
                break;
            case "darwin":
                this._platform = new macosplatform_1.default(platform);
                this._downloader = new downloader_1.default("mac", "x64", this._version, this._platform.platform);
                break;
            default:
    ...
    getQt(packages, deepSrc, flatSrc, cachedir, clean) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!toolPath) {
                ...
                toolPath = yield this.acquireQt(packages.split(";").map(s => s.trim()), deepSrc.split(";").map(s => s.trim()), flatSrc.split(";").map(s => s.trim()), cachedir);
            }
            else {
                cached = true;
    ...
    acquireQt(packages, deepSrc, flatSrc, cachedir) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let resDir;
            if (cachedir) {
                yield io.mv(path.join(installPath, vString, this._platform.platform), cachedir);
                resDir = path.resolve(cachedir);
            }
            else
                resDir = yield tc.cacheDir(path.join(installPath, vString, this._platform.platform), 'qt', vString, this._platform.platform);
            // move tools
            yield io.mv(path.join(installPath, "Tools"), path.join(resDir, "Tools"));
